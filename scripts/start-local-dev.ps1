<#
.SYNOPSIS
    RSP Dashboard - local production-like deploy for Windows.

.DESCRIPTION
    PowerShell mirror of start-local-dev.sh. Builds the Next.js client for
    production, starts the FastAPI backend on localhost, and serves the
    standalone frontend for local-only testing.
...
# Pin backend port before spawn. A stale $env:PORT=3001 from a prior frontend
# start in this shell would otherwise make uvicorn bind to the frontend port.
...
try {
    ...
    Write-Host "Local app URL: http://127.0.0.1:$FrontendPort"
