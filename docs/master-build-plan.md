...
## Phase 35 — Inspect Damage plotting upgrades (PU-35) (IN PROGRESS)

**Objective:** Replace the single 3D Inspect Damage plot surface with all primary 2D comparison cards plus one focused retained 3D card, keeping implementation frontend-only and low-entropy.

| Task ID | Task | Status | Key Files | Details |
|---------|------|--------|-----------|---------|
| PU-35-00 | Plotting upgrade PRD refresh, implementation map, and local issue breakdown | DONE (2026-06-15) | `docs/brainstorm/35_plotting_upgrades/PRD.md`, `docs/brainstorm/35_plotting_upgrades/IMPLEMENTATION_MAP.md`, `docs/brainstorm/35_plotting_upgrades/HANDOFF.md`, `docs/brainstorm/35_plotting_upgrades/issues/` | Captured resolved decision gates: all four 2D cards visible, one focused 3D card, no overlay rail, sidepanel Plot Inputs, subtle in-plot Normal/Log toggle, no version selector, one Reference scope and one Target scope. |
| PU-35-01 | Enforce one Reference/Target program-version scope in sidepanel selection | DONE (2026-06-15) | `client/src/components/dashboard/side-panel/ComparisonLoadDataSections.tsx`, `client/src/components/dashboard/side-panel/ComparisonLoadDataSections.test.tsx`, `docs/tasks/PU-35-01.md` | Added one-scope enforcement per side with same-scope multi-select support, cross-scope replace behavior, and explicit toast feedback when a side is moved to a new scope. |
| PU-35-02 | Move plot inputs into Inspect Damage sidepanel | DONE (2026-06-15) | `client/src/components/dashboard/side-panel/ComparisonLoadDataSections.tsx`, `client/src/components/dashboard/side-panel/ComparisonPlotInputsSection.tsx`, `client/src/components/dashboard/side-panel/ComparisonLoadDataSections.test.tsx`, `client/src/features/inspect-damage-3d/components/DamagePlotView.tsx`, `client/src/features/inspect-damage-3d/components/DamagePlotOverlayControls.tsx`, `docs/tasks/PU-35-02.md` | Added sidepanel `Plot Inputs` section below Target Load Data, moved selected-channel and value-mode controls to sidepanel-owned `DamageComparisonState`, enforced one-channel minimum there, and removed channel/value controls from the 3D overlay rail. |
| PU-35-03 | Extract shared plot card shell for reusable loading/error/empty chrome | DONE (2026-06-15) | `client/src/components/charts/PlotCardShell.tsx`, `client/src/components/charts/SVGPlotCard.tsx`, `client/src/components/charts/PlotCardShell.test.tsx`, `docs/tasks/PU-35-03.md` | Added reusable chart card shell with title/subtitle chip, loading/error/empty handling, and action/footer slots, then moved `SVGPlotCard` to consume the shell without changing existing empty-plot SVG behavior. |
| PU-35-04 | Add shared damage scale transform and in-plot toggle | DONE (2026-06-15) | `client/src/features/inspect-damage-3d/lib/damage-scale.ts`, `client/src/features/inspect-damage-3d/components/DamagePlotView.tsx`, `client/src/features/inspect-damage-3d/components/DamagePlotOverlayControls.tsx`, `client/src/features/inspect-damage-3d/__tests__/damage-scale.test.ts`, `client/src/features/inspect-damage-3d/components/DamagePlotView.test.tsx`, `docs/tasks/PU-35-04.md` | Added shared `applyDamageScale` utility (`linear` + `log10(1 + x)`), moved Normal/Log control into the main 3D plot area, and removed damage-scale ownership from the overlay rail while preserving existing focused 3D plot behavior. |
| PU-35-05 | Build cumulative-by-channel 2D plot spec | DONE (2026-06-15) | `client/src/features/inspect-damage-3d/lib/build-damage-2d-plot-spec.ts`, `client/src/features/inspect-damage-3d/__tests__/build-damage-2d-plot-spec.test.ts`, `docs/tasks/PU-35-05.md` | Added pure grouped-bar spec generation for `cumulative_by_channel` from aggregate channel totals with selected-channel filtering, absolute/normalized value support, linear/log scaling, semantic Reference/Target legend metadata, and explicit renderable empty-state handling. |
| PU-35-06 | Render cumulative-by-channel 2D card | DONE (2026-06-15) | `client/src/features/inspect-damage-3d/components/CumulativeByChannelPlotCard.tsx`, `client/src/features/inspect-damage-3d/components/CumulativeByChannelPlotCard.test.tsx`, `docs/tasks/PU-35-06.md` | Added grouped Reference/Target SVG bar renderer inside `PlotCardShell` with legend labels, native hover tooltip content, per-bar accessibility labels, and tested empty/loading/error shell states for cumulative channel specs. |
| PU-35-07 | Wire 2D card grid with focused 3D card | DONE (2026-06-15) | `client/src/features/inspect-damage-3d/components/DamagePlotView.tsx`, `client/src/features/inspect-damage-3d/components/Damage2DPlotCard.tsx`, `client/src/features/inspect-damage-3d/components/DamagePlotView.test.tsx`, `docs/tasks/PU-35-07.md` | Refactored Inspect Damage plot area into a 2×2 2D card grid plus one focused 3D card; cumulative-by-channel renders real grouped bars, other plot types use placeholder shells, 2D card clicks route focused 3D plot type, overlay rail removed, and cap warning/legend/Normal-Log toggle retained on focused 3D. |
| PU-35-08 | Add absolute-by-event heatmap card | DONE (2026-06-15) | `client/src/features/inspect-damage-3d/lib/build-damage-2d-plot-spec.ts`, `client/src/features/inspect-damage-3d/components/AbsoluteByEventHeatmapCard.tsx`, `client/src/features/inspect-damage-3d/components/Damage2DPlotCard.tsx`, `client/src/features/inspect-damage-3d/components/DamagePlotView.tsx`, `client/src/features/inspect-damage-3d/__tests__/build-damage-2d-plot-spec.test.ts`, `client/src/features/inspect-damage-3d/components/AbsoluteByEventHeatmapCard.test.tsx` | Added `absolute_by_event` 2D heatmap spec + renderer with selected event/channel filtering, value-mode + shared scale support, tooltip metadata, and graceful density summaries for large selections. |
| PU-35-09 | Add program/version comparison card | DONE (2026-06-15) | `client/src/features/inspect-damage-3d/lib/build-damage-2d-plot-spec.ts`, `client/src/features/inspect-damage-3d/components/Damage2DPlotCard.tsx`, `client/src/features/inspect-damage-3d/components/DamagePlotView.test.tsx`, `client/src/features/inspect-damage-3d/__tests__/build-damage-2d-plot-spec.test.ts`, `docs/tasks/PU-35-09.md` | Added `cumulative_by_program_version` spec support with selected-scope subtitle labels (Reference vs Target), value-mode/scale transforms, and scope-aware empty states; routed the card through the grouped-bar renderer so the 2D grid now shows real program/version totals instead of a placeholder shell. |
| PU-35-10 | Add signed target-delta card | DONE (2026-06-15) | `client/src/features/inspect-damage-3d/lib/build-damage-2d-plot-spec.ts`, `client/src/features/inspect-damage-3d/components/TargetDeltaVsReferencePlotCard.tsx`, `client/src/features/inspect-damage-3d/components/TargetDeltaVsReferencePlotCard.test.tsx`, `client/src/features/inspect-damage-3d/components/Damage2DPlotCard.tsx`, `client/src/features/inspect-damage-3d/components/DamagePlotView.test.tsx`, `client/src/features/inspect-damage-3d/__tests__/build-damage-2d-plot-spec.test.ts`, `docs/tasks/PU-35-10.md` | Added `target_delta_vs_reference` signed-delta spec + diverging renderer with zero baseline, selected-channel filtering, tooltip metadata (channel/reference/target/signed delta/value mode), and low-reference warning/flag propagation. |
| PU-35-11 | Add local delta metric toggle (Absolute / Percent / Ratio) | DONE (2026-06-15) | `client/src/features/inspect-damage-3d/components/TargetDeltaVsReferencePlotCard.tsx`, `client/src/features/inspect-damage-3d/lib/delta-metric-mode.ts`, `client/src/features/inspect-damage-3d/components/TargetDeltaVsReferencePlotCard.test.tsx`, `client/src/features/inspect-damage-3d/__tests__/delta-metric-mode.test.ts`, `docs/tasks/PU-35-11.md` | Added a card-local delta metric toggle in the target-delta 2D card, with mode-specific tooltip/axis formatting and low-reference suppression for percent/ratio outputs while preserving signed absolute delta behavior. |
| PU-35-12 | Polish empty states, a11y, and final plotting-upgrade QA pass | DONE (2026-06-15) | `client/src/app/database/page.tsx`, `client/src/app/inspect-damage/page.tsx`, `client/src/features/inspect-damage/components/DamageTableView.tsx`, `server/services/damage_inspect.py`, `server/models/damage.py`, `server/storage/database.py`, `docs/tasks/PU-35-12.md` | Moved persisted damage-table access under `/database` with `Datasets` and `Damage Table` tabs, added all-calculated inspect query mode (no manual selection required), defaulted version rows to collapsed in the damage table, and kept Inspect Damage route plot-focused. |
