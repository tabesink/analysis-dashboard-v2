...
export function DamagePlotAxes({ layout }: DamagePlotAxesProps) {
  ...
  return (
    <group>
      {/* Floor grid */}
      <gridHelper args={[Math.max(bounds.width, bounds.depth), Math.max(channelLabels.length, eventLabels.length), GRID_COLOR, GRID_COLOR]} position={[bounds.width / 2, 0, bounds.depth / 2]} />

      {/* Axis lines */}
      <line>
        <bufferGeometry attach="geometry" />
        <lineBasicMaterial attach="material" color={AXIS_COLOR} />
      </line>

      {/* Channel labels along Three.js X */}
      ...
      {/* Event labels along Three.js Z/depth; sample when many events exist. */}
