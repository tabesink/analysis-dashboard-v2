...
### Dialog Status Icons
- Dialog status badge: 32px circular container (`size-8`) on a muted surface, with a 1px border when the state needs extra containment.
- Inner Lucide glyph: 16px (`size-4`) for modal headers and summaries; inline row icons stay 14–16px.
- Error, warning, success, and progress states share the same compact geometry. Use semantic text color on the glyph, not a larger standalone symbol.
- Dialog summaries do not repeat the header title. The body starts with the explanatory message, then timing/count/detail metadata.
- Reserve 40px (`size-10`) icons for page-level empty states, large loading states, or illustration-like moments outside compact dialogs.
