...
### Changed
- Inspect Damage cumulative-by-channel plots now use per-channel max normalization in Normalized mode (`value / max(reference, target)` per channel), while damage-by-event plots keep share-of-total normalization; cumulative and event plots now use independent y-axis domains, with reference and target event plots sharing a common scale based on the larger stacked channel total across both datasets.
- Cross-plot pending-render hints and other short workflow info messages now use the shared Sonner toast helper (`showShortInfoToast`) instead of ad hoc inline banners or direct `toast.info` calls.
- Database and Inspect Damage data surfaces now use standardized shadcn table/card/filter shells with semantic status tokens on non-plot states (loading, running, stale, and error messaging), while preserving existing sorting/filtering/selection/expansion interactions and leaving dashboard/plot card internals unchanged (UI38-04).
...
- Removed remaining runtime database-import remnants from server/client operation lanes while preserving legacy import endpoint tombstones (`410 Gone`) and existing admin export/create/connect/delete contracts (OP39-01).
- Derived-data task authorization is now scope-based across poll/start/reuse paths: admins can poll/start/reuse any derived task scope, non-admin writers are limited to their uploaded program/version scopes, and unrelated writers cannot receive reused active task ids (OP39-02).
- Failed folder uploads now expose cleanup/retry guidance in task polling, and owner/admin users can run `POST /api/v1/upload/folder/task/{task_id}/cleanup` to remove only failed-task partial committed data so retrying the same files does not hit stale duplicate hashes (OP39-03).
- Operation admission guards now reject conflicting starts with structured `409` payloads: database switch/delete are blocked by active folder uploads, active derived tasks, active exports, or other exclusive database operations; create/export/upload/derived starts now also gate on exclusive database operations without adding external queue infrastructure (OP39-04).
- Authenticated clients now send lightweight active-user heartbeats; database switch is blocked when other active users are present on the current database with structured blocker usernames, and the settings switch dialog now shows a toast naming those active users when switch is denied (OP39-05).
- Active long-running progress dialogs are now non-dismissible across derived-data, folder upload, and database export flows: derived-data progress no longer offers “Close and continue in background,” active close requests are blocked until terminal state, and tests now cover active-vs-summary close behavior plus real cancel-path affordances (OP39-06).
- Final operation lifecycle golden-path verification now locks Phase 39 regression coverage across import-removal compatibility, cleanup/retry safety, scope authorization, operation/user switch blockers, and non-dismissible progress surfaces, while keeping structured status/blocker observability fields as the primary diagnostics contract (OP39-07).
- Upload-task lifecycle now treats `cancelling` as active and persists heartbeat/lifecycle observability fields (`started_at`, `cancel_requested_at`, `finished_at`, `last_heartbeat_at`, `runner_id`); stale active upload-task rows are reconciled at startup and request-time, operation admission reconciles before blocking database switch/delete, and upload task polling payloads expose lifecycle timing metadata (CU40-01).
- Upload-task cancellation now has a canonical server contract (`POST /api/v1/upload/tasks/{task_id}/cancel`) plus folder/derived compatibility aliases, shared owner/scope/admin authorization policy, idempotent `cancelling` intent persistence with `cancel_requested_at`, and typed client API cancel acknowledgement helpers/tests (CU40-02).
- Folder upload cancellation is now cooperative end-to-end: ingestion workers stop at safe checkpoints and terminalize as `cancelled` when cancel intent is accepted, cancelled partial uploads expose cleanup/retry guidance and can be cleaned via the same cleanup route as failed partial uploads, and the client cancel flow now sends server cancel requests and shows `Cancelling safely...` while polling until server terminal status (CU40-03).
- Reconnect recovery now uses durable server task state: `GET /api/v1/upload/tasks/active` returns creator-scoped folder uploads plus admin-or-scope-owner derived tasks (after stale-heartbeat reconciliation), Database startup restores recoverable progress surfaces, and auth/sync route behavior no longer depends on ephemeral `folderUploadInProgress` flags for cross-browser recovery (CU40-05).
- Auth login remains responsive under active dashboard DB write contention: login audit persistence now uses a bounded best-effort write path, login does not fail when dashboard audit insertion is unavailable/contended, and admission regressions now explicitly cover active/stale `cancelling` upload-task blockers for database switch/delete (CU40-06).
